...
class Parameter:
    ...
    """
    def __init__(self, name, nametex = None,
                 prior = [0, 1], **kwargs):
        self.name = name
        self.prior = prior
        if nametex:
...
gauss_model = ModelContainer([pos, width, height],
                             model = model, xdata = x, ydata = ydata,
                             std_noise = std_noise)

# because we want to `cat *.eggs > /dev/basket`
plot_model_kwargs = dict(linestyle = '-', c = 'blue', alpha = 0.3, lw = 0.1)
plot_data_kwargs = dict(drawstyle = 'steps', linestyle = '-',
                        c='k', label='data')
corner_kwargs = dict(labels = gauss_model.get_latex_names(),
                     show_titles = True, title_fmt = '.3f',
                     smooth = 0.5, bins = 40,
                     #plot_contours = False, plot_density = False,
                     quantiles = corner_quartiles, truths = real_model_pars)
...
corner.corner(a.data[:, 2:],
              fig = fig_corner, **corner_kwargs)
plt.show()
