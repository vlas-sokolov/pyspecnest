...
try:
    ...
    raise ImportError("Abandon the ship! PyMultiNest not found! "
...
def get_nh3_model(sp, lines, std_noise, priors = None, npeaks = 1):
    ...
    if priors is None:
        ...
        priors = [[5, 25], [2.7315, 10], [10, 17],
                  [0, 1], [0, 20], [0, 1]][::-1] * npeaks

    parlist = []
    for i in range(npeaks):
        tkin = Parameter("tkin_%i" % i, r'$\mathrm{T_{kin}}$', priors.pop())
        tex  = Parameter("tex_%i" % i, r'$\mathrm{T_{ex}}$', priors.pop()) # makes no sense setting tex > tkin
        ...
        o2p  = Parameter("o2p_%i" % i, r'$\frac{ortho-NH_3}{para-NH_3}$', priors.pop())
    ...
    nh3_model = ModelContainer(parlist, model = sp.specfit.get_full_model,
                               std_noise = std_noise, xdata = sp.xarr.value,
                               ydata = sp.data)
...
def get_plot_model_kwargs():
    plot_model_kwargs = dict(linestyle = '-', c = 'blue',
                             alpha = 0.3, linewidth = 0.1, zorder = 0.0)
...
def get_corner_kwargs(model, truepars = None):
    ...
    corner_kwargs = dict(labels = model.get_names(latex = True,
                                                  no_fixed = True),
...
def get_pymultinest_kwargs():
    pymultinest_kwargs = dict(sampling_efficiency = 0.8,
                              #resume = False,
                              verbose = True)
...
def synthetic_nh3_inference(sp = None, std_noise = None, npeaks = 1,
    ...
    * Plots the global posterior parameter distribution
    ...
    if sp is None:
        ...
        sp = nh3_spectrum(truepars, xvals, seed = None,
                          lines = lines, snr = snr,
                          snr_line = 'twotwo')
        std_noise = sp.error.mean()

    nh3_model = get_nh3_model(sp, lines, std_noise,
                              priors = priors,
                              npeaks = npeaks)
